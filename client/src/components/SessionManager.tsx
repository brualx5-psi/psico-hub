...
export const SessionManager: React.FC = () => {
    ...
    const handlePresent = (session: ScheduledSession) => {
        ...
        const newPayment: PaymentRecord = {
            ...
            amount,
            status: isInsurance ? 'a_receber' : 'pendente',
            method: isInsurance ? 'convenio' : billing?.paymentType === 'pacote' ? 'pacote' : undefined,
            expectedDate: isInsurance && insurance
                ? new Date(Date.now() + (insurance.reimbursementDays || 60) * 24 * 60 * 60 * 1000).toISOString()
                : undefined,
            notes: `Sessão ${new Date(session.scheduledDate).toLocaleDateString('pt-BR')}`,
        ...
        const updatedSessions = sessions.map(s =>
            s.id === session.id
                ? { ...s, status: 'presente' as const, attendanceConfirmedAt: new Date().toISOString(), paymentId }
                : s
        );
    ...
    const handleAbsent = (session: ScheduledSession, reason?: string) => {
        ...
        const updatedSessions = sessions.map(s =>
            s.id === session.id
                ? { ...s, status: 'falta' as const, attendanceConfirmedAt: new Date().toISOString(), absenceReason: reason }
                : s
        );

        // Create automatic evolution note
        const absenceNote = {
            id: crypto.randomUUID(),
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Presença e agendamento de {currentPatient.name}</p>
                </div>
